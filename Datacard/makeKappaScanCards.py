...
def duplicateColumn(data,shapeData,src_ch,src_proc,dst_ch,dst_proc,lumi=None,isSig=False):
    for ch in data:
        ...
        pid=0
        for proc_ in data[ch]:
            if isSig and (not isSignalProc(proc_)):
                continue
            if (not isSig) and  isSignalProc(proc_):
                continue
            pid=max(abs(data[ch][proc_]['id']),pid)
        pid+=1
        #print(src_ch,src_proc,dst_ch,dst_proc,pid)
        if isSig:
            pid*=-1
        if dst_ch in data:
...
def getShapeStringBlock(shapeData):
    ...
    print(f"Number of observations exported : [{l}] ")
...
def getProcYieldStringBlock(dataS): 
    ...
    print()
    print(f"Signals exported : [{sStr}]")
    print()
    print(f"Backgrounds exported : [{bStr}]")
